#!/usr/bin/env bash
# Tier-1: minimap 定位框必须与窗口保持同一长宽比（issue 的核心契约）。
set -euo pipefail
cd "$(dirname "$0")/.."

mkdir -p verify_tier1
cat > verify_tier1/minimap_tier1.ts <<'EOF'
...
rm -rf /tmp/gtv-minimap-tier1 && mkdir -p /tmp/gtv-minimap-tier1
./node_modules/.bin/tsc verify_tier1/minimap_tier1.ts src/components/minimap.ts \
  --outDir /tmp/gtv-minimap-tier1 --rootDir . --module commonjs --target es2020 \
  --lib es2020,dom --strict --skipLibCheck
node /tmp/gtv-minimap-tier1/verify_tier1/minimap_tier1.js
