...
- 用户再修一版交互契约（6be62fd）：**高亮/悬停/单击 = 仅预览**（面板跟
  随、视图不动、下拉不关），**双击/Enter = 定位**（居中或跳转 + 详情 +
  关下拉）。原单击即定位的设计在"扫结果"场景下每点一次就丢上下文。
  空闲态 footer 常驻操作提示（↑↓/悬停/单击预览 · 双击或 Enter 定位），
  双击手势否则不可发现。验证：单击后 svg transform 原封不动、双击后
  transform.x = 640 − x_target 精确居中。
