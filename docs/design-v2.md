...
- **fork 线**：圆角肘形曲线——从父泳道节点以竖直切线出发（垂落点仍与派生提交
  时间对齐），平滑弯转后以水平切线沿空轨道接入子泳道首节点；**merge 线**：镜像的
  圆角肘形曲线汇入主线 merge 节点，视觉权重明显低于泳道横杠与 fork 线。
...
- **fork 出生线 / merge 汇合线**：可走空轨道时用圆角肘形曲线 + 出生点/收拢点标注
  （泳道名，9px，泳道色）；被合并分支仍存活、出生点错位等走不了肘形的情况用
  对角切线前向曲线——两端约 45° 切线，x 全程单调前进，保证所有连接线与时间轴
  同向（左→右），一眼看出 commit 先后；时钟倒错（rebase/amend 造成）的边反向
  绘制，如实反映数据。
